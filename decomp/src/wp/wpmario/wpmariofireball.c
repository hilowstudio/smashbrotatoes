...
#ifdef PORT
    /* Same class as the Samus charge-shot bypass fix: on the spawn frame the
     * pooled weapon's attack_pos[0].pos_prev is the pool default (0,0,0) or a
     * stale value left by a recycled prior fireball, while pos_curr is the
     * muzzle. A frame-0 hit test then sweeps a bogus segment (from the origin or
     * a recycled position) that can mis-collide. Drive the New->Transfer->
     * Interpolate machine here so pos_prev = pos_curr = spawn (a zero-length
     * segment) on the spawn frame; the fireball still hits normally as it
     * travels (the generic weapon process ticks it each subsequent frame).
     * The old "fireball passes through the opponent" symptom is otherwise gone
     * on its own (runtime-verified: fireballs register hits for real damage);
     * this only removes the frame-0 stale-sweep hazard. Mirrors the same fix in
     * wpSamusChargeShotMakeWeapon. */
    wp->attack_coll.attack_state = nGMAttackStateNew;
    wpProcessUpdateHitPositions(weapon_gobj);   /* New -> Transfer: sets pos_curr */
    wpProcessUpdateHitPositions(weapon_gobj);   /* Transfer -> Interpolate: pos_prev = pos_curr */
#endif
