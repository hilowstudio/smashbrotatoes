...
GObj* wpSamusChargeShotMakeWeapon(GObj *fighter_gobj, Vec3f *pos, s32 charge_level, sb32 is_release) 
{
    ...
    if (is_release != FALSE)
    {
        wpSamusChargeShotLaunch(weapon_gobj);
#ifdef PORT
        /* The DK-vs-Samus intro cinematic (scene 45) taps B back-to-back, which
         * takes this is_release=TRUE bypass and skips the normal charging->fire
         * flow (wpSamusChargeShotProcUpdate, which sets attack_state=New and ticks
         * wpProcessUpdateHitPositions). Left uninitialised, attack_pos[0].pos_prev
         * stays at the pool default (0,0,0) while pos_curr is the muzzle, so the
         * swept hit test sweeps a segment from the WORLD ORIGIN across the level
         * and mis-collides. Drive the New->Transfer->Interpolate machine here so
         * both pos_prev and pos_curr equal the spawn position (a zero-length
         * segment at the muzzle) on the fire frame, exactly as the normal path
         * ends up. See docs/bugs/samus_charge_shot_hit_detection. */
        wp->attack_coll.attack_state = nGMAttackStateNew;
