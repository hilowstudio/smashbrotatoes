...
/* MSVC (and clang-cl) ignore the `#pragma weak sinf = __sinf` / `cosf = __cosf`
 * aliases in libultra's gu/sinf.c & gu/cosf.c, so the bare sinf()/cosf() decomp
 * call sites would otherwise bind the host UCRT libm instead of libultra's
 * Cody-Waite polynomial — silently diverging physics/camera/particle trig from
 * console (GCC/Clang capture them via the weak aliases). Remap the bare names
 * to the polynomial here: gu.h is the sole declaration site for sinf/cosf, so
 * every caller includes it and the redirect is always in scope. The N64/IDO
 * build (PORT undefined) keeps the original prototypes untouched.
 * See docs/bugs/trig_host_libm_divergence_2026-07-31.md. */
extern float __sinf(float angle);
