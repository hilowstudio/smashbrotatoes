...
void Interpreter::ImportTextureIA16(int tile, bool importReplacement) {
    ...
    uint32_t widthBytes = GetEffectiveLineSize(line_size_bytes, full_image_line_size_bytes, size_bytes,
                                               mRdp->texture_tile[tile].line_size_bytes);
    // naturalWidth = pre-clamp TMEM-line-derived pixel count; stays the source
    // stride for the decode below. `width` (post-clamp) is the upload/decode
    // extent. See docs/bugs/ia4_upload_unclamped_to_tile_2026-04-29.md — IA16
    // was one of the latent unclamped paths; without the clamp a sprite whose
    // TMEM line stride is wider than the SetTileSize extent renders squished.
    uint32_t naturalWidth = widthBytes / 2;
    uint32_t height = widthBytes > 0 ? size_bytes / widthBytes : 0;

    // A single line of pixels should not equal the entire image (height == 1 non-withstanding)
    if (full_image_line_size_bytes == size_bytes) {
        full_image_line_size_bytes = naturalWidth * 2;
    }
...
void Interpreter::ImportTextureI8(int tile, bool importReplacement) {
    ...
    // naturalWidth = pre-clamp TMEM-line-derived pixel count (1 byte/px for I8);
    // stays the source stride below. `width` (post-clamp) is the upload/decode
    // extent. See docs/bugs/ia4_upload_unclamped_to_tile_2026-04-29.md — I8 was
    // one of the latent unclamped paths; without the clamp a sprite whose TMEM
    // line stride is wider than the SetTileSize extent renders squished. Mirrors
    // ImportTextureIA8.
    uint32_t naturalWidth = GetEffectiveLineSize(lineSizeBytes, fullImageLineSizeBytes, sizeBytes,
                                                 mRdp->texture_tile[tile].line_size_bytes);
    uint32_t height = naturalWidth > 0 ? sizeBytes / naturalWidth : 0;
...
// PORT: N64 16bpp YUV texture decode. Upstream Fast3D discarded these with an
// SPDLOG_ERROR, leaving SSB64's YUV sprites black. N64 YUV16 packs two pixels
// per 32 bits as [Y0 U Y1 V] (YUYV): within each 4-byte group the U and V
// chroma bytes are shared by the two luma samples Y0/Y1. The RDP color-convert
// unit turns (Y, U, V) into RGB with the signed 9-bit K0..K3 coefficients set by
// gDPSetConvert (G_SETCONVERT), scaled by 1/128 (>>7):
//   R = Y + (K0 * (V-128)) >> 7
//   G = Y + (K1 * (U-128) + K2 * (V-128)) >> 7
//   B = Y + (K3 * (U-128)) >> 7
// with U=Cb, V=Cr centered at 128, each channel clamped to [0,255], alpha=255.
// The RDP hardware defaults (G_CV_K0..K3 = 175, -43, -89, 222) are the standard
// BT.601-ish coefficients (K0/128≈1.37 for Cr, K3/128≈1.73 for Cb, etc.); if the
// game never issued G_SETCONVERT we fall back to those.
//
// RDP-semantics assumptions documented for review:
//  * The chroma multiply uses the coefficients with U/V centered at 128. This is
//    inferred from the default G_CV_K* matching BT.601 under a 1/128 scale; the
//    real RDP applies the convert unit at the texture-filter output, but for an
//    unfiltered per-texel decode this reproduces the intended colors.
//  * `(k * chroma) >> 7` relies on arithmetic right shift of a signed intermediate
//    (all supported MSVC/Clang/GCC targets implement it as arithmetic), matching
//    the RDP's signed fixed-point scaling.
void Interpreter::ImportTextureYuv16(int tile, bool importReplacement) {
    const RawTexMetadata* metadata = &mRdp->loaded_texture[mRdp->texture_tile[tile].tmem_index].raw_tex_metadata;
    const uint8_t* addr =
        importReplacement && (metadata->resource != nullptr)
            ? GetMaskedReplacementData(mMaskedTextures, GetBaseTexturePath(metadata->resource->GetInitData()->Path))
    ...
    if (addr == nullptr) {
        SPDLOG_ERROR("ImportTextureYuv16: null texture address for tile {}", tile);
        return;
    }
    ...
    uint32_t widthBytes = GetEffectiveLineSize(line_size_bytes, full_image_line_size_bytes, size_bytes,
                                               mRdp->texture_tile[tile].line_size_bytes);
    // 2 bytes per pixel (one luma + one shared-chroma byte). naturalWidth stays
    // the source stride; `width` is the clamped upload extent, mirroring the
    // ClampUploadWidthToTile pattern used by the sibling 16bpp decoders.
    uint32_t naturalWidth = widthBytes / 2;
    uint32_t height = widthBytes > 0 ? size_bytes / widthBytes : 0;
    ...
    // Signed convert coefficients: game-supplied if G_SETCONVERT was issued,
    // otherwise the fixed RDP defaults.
    int32_t k0 = mRdp->convert_set ? mRdp->convert_k[0] : G_CV_K0;
    int32_t k1 = mRdp->convert_set ? mRdp->convert_k[1] : G_CV_K1;
    int32_t k2 = mRdp->convert_set ? mRdp->convert_k[2] : G_CV_K2;
    int32_t k3 = mRdp->convert_set ? mRdp->convert_k[3] : G_CV_K3;

    auto clamp8 = [](int32_t v) -> uint8_t { return v < 0 ? 0 : (v > 255 ? 255 : (uint8_t)v); };

    uint32_t stridePixels = full_image_line_size_bytes / 2;
    uint32_t i = 0;
    for (uint32_t y = 0; y < height; y++) {
        for (uint32_t x = 0; x < width; x++) {
            // Byte layout within a row: pixel p occupies bytes [2p, 2p+1]; the
            // shared chroma pair lives in the even pixel of each 2-pixel group.
            uint32_t rowBaseBytes = y * stridePixels * 2;
            uint32_t pairEvenPixel = x & ~1u;
            uint8_t yLuma = addr[rowBaseBytes + x * 2];
            uint8_t uChroma = addr[rowBaseBytes + pairEvenPixel * 2 + 1]; // Cb
            uint8_t vChroma = addr[rowBaseBytes + pairEvenPixel * 2 + 3]; // Cr

            int32_t yv = yLuma;
            int32_t cb = (int32_t)uChroma - 128;
            int32_t cr = (int32_t)vChroma - 128;

            mTexUploadBuffer[4 * i + 0] = clamp8(yv + ((k0 * cr) >> 7));
            mTexUploadBuffer[4 * i + 1] = clamp8(yv + ((k1 * cb + k2 * cr) >> 7));
            mTexUploadBuffer[4 * i + 2] = clamp8(yv + ((k3 * cb) >> 7));
    ...
    if (!importReplacement) {
        if (GfxHiResHookFn hook = gHiResHook) {
            const uint8_t* packBuf = nullptr;
            uint16_t packW = 0, packH = 0;
            if (hook(G_IM_FMT_YUV, G_IM_SIZ_16b, mTexUploadBuffer, (uint16_t)width, (uint16_t)height,
                     &packBuf, &packW, &packH)
                && packBuf != nullptr && packW > 0 && packH > 0) {
                mRapi->UploadTexture(packBuf, packW, packH);
                return;
...
void Interpreter::ImportTexture(int i, int tile, bool importReplacement) {
    ...
    switch (fmt) {
        ...
        case G_IM_FMT_YUV:
            if (siz == G_IM_SIZ_16b) {
                ImportTextureYuv16(tile, importReplacement);
            } else {
                SPDLOG_ERROR("YUV Texture invalid siz={} tile={}", siz, tile);
            }
            break;
        default:
...
void Interpreter::Gfxs2dexBg1cyc(F3DuObjBg* bg) {
    ...
    // Background scaling: honour the S2DEX uObjScaleBg scaleW/scaleH (u5.10,
    // where 1024 == 1.0 == one texel per pixel) as the RDP texture-rectangle
    // dsdx/dtdy — the same mapping the sprite path (Gfxs2dexRecyCopy) uses. This
    // was previously hardcoded to identity (1<<10), which only renders correctly
    // when the background is drawn 1:1. scaleW/scaleH are already the same
    // fixed-point as dsdx/dtdy, so they pass through directly. Fall back to
    // identity if the object left scale unset (0) so a 1:1 background can never
    // collapse to a single-texel smear (dsdx=0). See docs/audit_fast3d_gaps §3.
    s16 uls = bg->b.imageX >> 2;
    s16 lrs = bg->b.imageW >> 2;

    s16 dsdxRect = bg->s.scaleW != 0 ? (s16)bg->s.scaleW : (s16)(1 << 10);
    s16 dtdyRect = bg->s.scaleH != 0 ? (s16)bg->s.scaleH : (s16)(1 << 10);
    ...
    GfxDpTextureRectangle(bg->b.frameX, bg->b.frameY, bg->b.frameW, bg->b.frameH, G_TX_RENDERTILE, ulsRect,
                          bg->b.imageY << 3, dsdxRect, dtdyRect, false);
}
...
bool gfx_set_convert_handler_rdp(F3DGfx** cmd0) {
    Interpreter* gfx = mInstance.lock().get();
    F3DGfx* cmd = *cmd0;
    // gDPSetConvert packs the six signed 9-bit YUV->RGB coefficients K0..K5 as
    // (see libultra/gbi.h):
    //   w0 = _SHIFTL(op,24,8) | _SHIFTL(k0,13,9) | _SHIFTL(k1,4,9) | _SHIFTR(k2,5,4)
    //   w1 = _SHIFTL(k2,27,5) | _SHIFTL(k3,18,9) | _SHIFTL(k4,9,9)  | _SHIFTL(k5,0,9)
    // so k2's high 4 bits ride in w0[3:0] and its low 5 bits in w1[31:27].
    uint32_t w0 = (uint32_t)cmd->words.w0;
    uint32_t w1 = (uint32_t)cmd->words.w1;
    uint32_t raw[6];
    raw[0] = (w0 >> 13) & 0x1FF;
    raw[1] = (w0 >> 4) & 0x1FF;
    raw[2] = ((w0 & 0xF) << 5) | ((w1 >> 27) & 0x1F);
    raw[3] = (w1 >> 18) & 0x1FF;
    raw[4] = (w1 >> 9) & 0x1FF;
    raw[5] = w1 & 0x1FF;
    for (int k = 0; k < 6; k++) {
        // Sign-extend the 9-bit two's-complement coefficient to int16_t.
        gfx->mRdp->convert_k[k] = (int16_t)((raw[k] ^ 0x100) - 0x100);
    }
    gfx->mRdp->convert_set = true;
    return false;
}
...
static void gfx_step() {
    ...
    /* Unresolved-N64-address deref guard (variant 4 in
     * docs/bugs/linux_stale_scene_data_family_2026-05-11.md). The G_DL push-time
     * bounds check in gfx_dl_handler_common deliberately lets sub-0x0FFFFFFF
     * addresses through — a segment may bind on a later frame, and rejecting
     * there would false-drop legitimate segment-relative sub-DLs (see the long
     * rationale at that check). So a gsSPDisplayList that pushed a raw, still-
     * unresolved N64 segment address (e.g. 0x0E000000, segment 0x0E unbound and
     * no containing file) reaches the walker here as `cmd`.
     *
     * On a 64-bit host no valid F3DGfx* is ever <= 0x0FFFFFFF (real heap/module
     * addresses are far higher), so such a `cmd` is unambiguously an unresolved
     * N64 token about to be dereferenced as an opcode at the line below — a
     * guaranteed fault on Linux/glibc (the reported SIGSEGV) or a garbage read
     * on Win/Mac. Stop the walk cleanly instead, exactly as the walked-past
     * guard above does: this frame's remaining draws are lost but the next frame
     * recovers. Guarded to 64-bit so it can never false-positive (drop a
     * renderable command) on a 32-bit host where low addresses are valid.
     * DumpDLDiag only classifies/prints the address (never derefs it), so it is
     * safe here and its segment-table + recent-DL-push dump is what pins down
     * which DL emitted the unresolved reference upstream. */
#if UINTPTR_MAX > 0xFFFFFFFFu
    if ((uintptr_t)cmd <= 0x0FFFFFFFu) {
        static int sUnresolvedCmdCount = 0;
        if (sUnresolvedCmdCount < 10) {
            sUnresolvedCmdCount++;
            SPDLOG_ERROR("gfx_step: cmd=0x{:x} is an unresolved N64 segment address "
                         "(<= 0x0FFFFFFF) — stopping walk before deref (a gsSPDisplayList "
                         "pushed an unbound-segment sub-DL; see variant 4)",
                         (unsigned long long)(uintptr_t)cmd);
            Fast::DumpDLDiag(cmd, "gfx_step: unresolved N64 cmd address");
        }
        g_exec_stack.stop();
        return;
    }
