...
void ScanShaderDir(const std::filesystem::path& root, const std::string& relPrefix,
    ...
    // Step the iterator with an error_code so a mid-scan failure (a removable
    // drive going away, a permission change, etc.) ends iteration instead of
    // throwing std::filesystem_error. A range-for would use the throwing
    // operator++ even though the iterator was constructed with an ec.
    std::filesystem::directory_iterator dirIt(root, ec);
