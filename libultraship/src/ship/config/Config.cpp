...
void Config::Reload() {
    // Use the noexcept (p, ec) overloads. The throwing overloads raise
    // std::filesystem_error on non-ENOENT failures (e.g. a drive that is
    // present but has no media -> ERROR_NOT_READY on Windows), which would
    // unwind out of the Config constructor at boot with no catch in scope.
    // For a config-path probe, treating any failure as "absent" is correct:
    // we fall through to a fresh in-memory instance.
    std::error_code ec;
    if (mPath == "None" || !fs::exists(mPath, ec) || !fs::is_regular_file(mPath, ec)) {
        mIsNewInstance = true;
