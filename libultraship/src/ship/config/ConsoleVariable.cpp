...
void ConsoleVariable::LoadLegacy() {
    ...
    if (DiskFile::Exists(conf)) {
        ...
        // Fire-and-forget cleanup of the legacy config. Use the noexcept
        // (p, ec) overload so a failure to delete (permission, no-media drive,
        // etc.) can't throw std::filesystem_error out of this boot-time load.
        std::error_code ec;
        fs::remove(conf, ec);
    }
}
