...
std::vector<std::string> ArchiveManager::GetArchiveListInPaths(const std::vector<std::string>& archivePaths) {
    ...
    // absolute() throws on failure; for a display/list string, fall back to the
    // raw path rather than terminating.
    auto absOrRaw = [](const std::filesystem::path& p) -> std::string {
        std::error_code ec;
    ...
    for (const auto& archivePath : archivePaths) {
        if (archivePath.length() > 0) {
            // Use the noexcept (p, ec) overloads throughout. The throwing
            // overloads raise std::filesystem_error on non-ENOENT failures
            // (e.g. a present-but-no-media drive -> ERROR_NOT_READY on Windows),
            // which would unwind out of this boot-time archive scan with no
            // catch in scope. For path probes, treating any failure as
            // "absent" preserves the original control flow.
            std::error_code ec;
            if (std::filesystem::is_directory(archivePath, ec)) {
                bool hasAssetFiles = false;
                // Construct and step the iterator with an error_code so a
                // mid-scan failure ends iteration instead of throwing.
                std::error_code dirEc;
                std::filesystem::directory_iterator it(archivePath, dirEc);
                const std::filesystem::directory_iterator end;
                for (; !dirEc && it != end; it.increment(dirEc)) {
                    const auto& p = *it;
                    if (StringHelper::IEquals(p.path().extension().string(), ".otr") ||
                        ...
                        fileList.push_back(absOrRaw(p.path()));
                        hasAssetFiles = true;
                    }
                }

                if (!hasAssetFiles) {
                    fileList.push_back(absOrRaw(archivePath));
                }
            } else if (std::filesystem::is_regular_file(archivePath, ec)) {
                fileList.push_back(absOrRaw(archivePath));
            } else {
                SPDLOG_WARN("The archive at path {} does not exist", absOrRaw(archivePath));
            }
        } else {
