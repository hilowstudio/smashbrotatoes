...
std::optional<std::filesystem::path> FindLibraryFile(const std::vector<std::string>& libraryPaths,
    ...
    // Use the noexcept (p, ec) overloads throughout: the throwing overloads
    // raise std::filesystem_error on non-ENOENT failures (e.g. a no-media drive
    // on Windows), which would unwind out of the script loader. For these
    // probes, treating any failure as "not found" preserves the search flow.
    std::error_code ec;
    std::filesystem::path libraryPath(library);
    if (libraryPath.is_absolute() && std::filesystem::exists(libraryPath, ec)) {
        return libraryPath;
    }

    if (std::filesystem::exists(libraryPath, ec)) {
        std::filesystem::path abs = std::filesystem::absolute(libraryPath, ec);
        return ec ? libraryPath : abs;
    }
...
void SetTccLibraryRoot(TCCState* state, const std::vector<std::string>& libraryPaths) {
    ...
    if (tccRoot.empty() || !std::filesystem::exists(tccRoot, ec)) {
        return;
    }
...
void ScriptLoader::Compile(const std::shared_ptr<Archive>& archive) {
    ...
    if (binaries.contains(std::string(platform))) {
        ...
        for (const std::string& includePath : mIncludePaths) {
            if (!std::filesystem::exists(includePath, pathEc)) {
                SPDLOG_WARN("Include path does not exist: {}", includePath);
                continue;
            }

            if (!std::filesystem::is_directory(includePath, pathEc)) {
        ...
        for (const std::string& libraryPath : mLibraryPaths) {
            if (!std::filesystem::exists(libraryPath, pathEc)) {
                SPDLOG_WARN("Library path does not exist: {}", libraryPath);
                continue;
            }

            if (!std::filesystem::is_directory(libraryPath, pathEc)) {
