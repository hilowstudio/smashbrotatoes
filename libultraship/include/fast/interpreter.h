...
struct RDP {
    ...
    // PORT: G_SETCONVERT stores the YUV->RGB color-convert coefficients K0..K5.
    // Each is a signed 9-bit value (two's-complement, range [-256, 255]). K0-K3
    // scale the chroma terms of the RDP's fixed-point YUV->RGB conversion used by
    // both YUV texture decode and the G_CC_*YUV2RGB color-combine path; K4/K5 are
    // an offset/scale pair consumed only by the color-combine YUV path (not the
    // texture decoder). Upstream Fast3D stubbed G_SETCONVERT, leaving YUV textures
    // undecodable. `convert_set` records whether the game issued G_SETCONVERT so
    // the texture decoder can fall back to the fixed RDP defaults (G_CV_K*).
    int16_t convert_k[6];
    bool convert_set;
};
...
class Interpreter {
  public:
    ...
    void ImportTextureYuv16(int tile, bool importReplacement);
