...
**Note**: FTModelPart and FTAccessPart (in `src/ft/fttypes.h`) are also
file-data pointer-bearing structs. They were discovered during Phase 1 and
deferred at the time because they carry additional pointer fields (mobjsubs,
matanim_joints). **UPDATE (2026-08-19): DONE.** All pointer fields in
`FTAccessPart` (dl, mobjsubs, costume_matanim_joints), `FTModelPart` (dl,
mobjsubs, costume_matanim_joints, main_matanim_joints), and `FTCommonPart`
(dobjdesc, p_mobjsubs, p_costume_matanim_joints) are now `u32` tokens under
`#ifdef PORT`, with `FTACCESSPART_GET_*` / `FTMODELPART_GET_*` / `FTPARTS_GET_*`
`PORT_RESOLVE` accessor macros and `_Static_assert` size checks
(FTAccessPart==16, FTModelPart==20, FTCommonPart==16). 32 access sites use the
macros; no raw token-field derefs remain.
