// We own `int main` directly, so suppress SDL's `#define main SDL_main`.
...
#define SSB64_APP_NAME "BattleShip"
	...
	// See controlDeck note above re: scoping.
	{
		...
		// The .ico is baked into the .exe as a Win32 resource, so the
		// runtime SDL_SetWindowIcon path is a no-op on Windows.
		ssb64::SetWindowIcon();
	}
