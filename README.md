...
**SmashBrotatoes** is a PC port of **Super Smash Bros. (N64)** — the **US** (NTSC-U v1.0) release — built on top of the [VetriTheRetri/ssb-decomp-re](https://github.com/vetritheretri/ssb-decomp-re) decompilation, using [libultraship](https://github.com/Kenix3/libultraship) for PC-native rendering / audio / input and [Torch](https://github.com/HarbourMasters/Torch) for extracting assets out of the ROM at build time.
...
You supply your own ROM (see [BUILDING.md](BUILDING.md)). The canonical,
supported dump (internal name `SMASH BROTHERS`):

| Version | Game code | SHA‑1 | MD5 |
|---------|-----------|-------|-----|
| **US** — NTSC-U v1.0 | `NALE` | `e2929e10fccc0aa84e5776227e798abc07cedabf` | `f7c52568a31aadf26e14dc2b6416b2ed` |

If your dump does not match these hashes, it will not work.
...
A set of files is generated by the Python tools in `tools/` from the tracked symbol source `tools/reloc_data_symbols.us.txt`, and **committed to the repo** so a fresh clone and CI build without re-running the generators (and, for the YAMLs, without the ROM). Regenerate and re-commit only when the symbol source changes — rare for a frozen v1.0 ROM:

- `include/reloc_data.us.h` — extern declarations for every relocatable symbol, included via the `decomp/include/reloc_data.h` shim (`REGION_US`)
- `port/resource/RelocFileTable.us.cpp` — the runtime symbol table
...
The CMake targets `GenerateRelocArtifacts` and `RegenerateRelocYamls` rebuild them on demand. If you edit `reloc_data_symbols.us.txt`, regenerate and re-commit; a stale table surfaces as an "undefined reference to `dFooBarReloc`" link error.
